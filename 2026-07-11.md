...
내가 지금 서 있는 브랜치는 삭제할 수 없다. (별도 워크트리가 있는 건 아니고, git은 "현재 작업 중인 폴더" 자체를 worktree라고 부른다.) 다른 브랜치로 발을 옮긴 뒤 지워야 한다.
...
내가 앉아 있는 의자는 내가 뺄 수 없고, 옆 의자로 옮겨 앉은 뒤에야 뺄 수 있는 셈.
...
### 10. (추가 공부) 과거 Claude Code 브랜치 사고 = 오늘 배운 그것이었다

이건 남 얘기가 아니었다. 전에 맥·데스크탑 두 기기로 Claude Code 작업을 동기화하다 `main`이 꼬여 고생한 적이 있는데, 오늘 개념으로 다시 보니 정체가 잡혔다.

- **두 기기가 같은 `main`에 각각 커밋한 것 = 오늘 두 브랜치가 공통 조상에서 갈라진 것과 똑같은 상황**이다. 브랜치 이름만 같을 뿐 서로 다른 갈래였던 셈.
- 그때 `push`가 거부됐던 게(`non-fast-forward`) 이제 읽힌다: 원격이 더 이상 내 로컬의 조상이 아니게 갈라져서, git이 **"이건 fast-forward가 안 된다"**며 덮어쓰기를 거부한 것. fast-forward를 배우니 거부 사유가 그대로 해석된다.
- 내 작업공간 규칙 **"⚠갈라짐이면 push 금지 → `git pull --rebase`"**도 정확히 오늘의 rebase였다: 아직 push 안 한 내 로컬 커밋을 원격 최신 위로 다시 심어 일직선으로 만든 뒤 fast-forward로 밀어넣는 것. "아직 안 나눈 커밋이라 rebase해도 안전"이라는 점이 §9와 딱 맞물린다.
- 어제 "브랜치는 워크트리에 묶인다(발밑 파일이 바뀐다)"와 규칙 "공유 트리에선 브랜치 전환 금지"도 한 줄로 이어진다.

결국 그동안 **규칙이라 외워서 따르던** "pull 먼저, 갈라짐이면 pull --rebase"가 merge·rebase·fast-forward라는 기계장치의 응용이었다. 사고는 갈라짐을 모른 채 만든 데서 났고, 오늘 비로소 그 이름과 해법을 손에 쥐었다.
...
```
  ...
  - **커밋이 `master`가 아니라 `main`에 올라갔다**(`[main 34d5eb3]`). 위 5번(프롬프트-실제 어긋남)의 결과 — 내가 있다고 생각한 브랜치와 실제 커밋된 브랜치가 달랐다. 커밋 전 브랜치 확인이 왜 필요한지 몸으로 겪은 셈.
...
- 해결: 에디터에서 저장 → `git add .` → 커밋되니 `[add-coach 61a1c4a]`. **"저장 → add → commit" 3박자**를 몸에 새길 것.
...
## 오늘의 회고
- 몰입도: 갈라놓은 세 갈래를 merge·rebase로 직접 합쳐보니 개념이 손에 잡혔다. 특히 fast-forward는 강의만으론 안 잡혀 따로 찾아봤는데, rebase가 만든 일직선 덕에 자동으로 일어난 걸 그래프로 확인하고 나서야 이해됐다.
- 오늘의 수확: 배운 걸 **내 현실 문제와 연결**한 것. 예전 두 기기 Claude Code main 충돌이 오늘의 "갈라짐"이었고, 규칙으로만 외우던 `pull --rebase`가 오늘의 rebase였다는 걸 알게 된 게 제일 크다.
