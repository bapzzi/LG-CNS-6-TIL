...
> TL;DR: 리액트 강의 마무리(4·5강). (1) **CSS**: 클래스 선택자는 `.클래스명`으로 적용한다. body 배경 그라디언트가 안 먹었는데, 원인은 `background-color`에 그라디언트를 넣은 것이었다 — 그라디언트는 이미지라 `background`로 줘야 한다. (2) **라우팅·통신**: 페이지 이동엔 `react-router-dom`, 백엔드 통신엔 `axios`가 필요하다. 둘을 설치하고, 로그인 로직에 입력 검증·`async`·(주석 처리된) API 호출을 붙였다. `App.js`에 라우터(`/`→Login, `/welcome`→Welcome)를 연결하고, Welcome 페이지의 오타·문법 오류를 잡아 `/`↔`/welcome` 이동을 확인했다.
...
- 새로 만든 `Welcome.jsx`는 처음엔 오타·문법 오류가 겹쳐 화면이 통째로 깨졌다. 아래 트러블슈팅에서 잡았고, 지금은 `/welcome` 접속과 로그아웃(→`/`)까지 라우터가 실제로 돈다.
...
### 2. Welcome 페이지가 통째로 깨진다 — 오타·문법 오류 모음

- **문제**: 라우터로 `/welcome`에 연결한 `Welcome.jsx`가 화면을 아예 못 그리고 오류가 여럿 떴다.
- **원인**: 한 줄씩 보니 잔오류가 겹쳐 있었다.
  - `import { useNavigate, userLocation }` — 훅 이름 **오타**. `useLocation`인데 `userLocation`으로 적어, 존재하지 않는 걸 import했다.
  - `const { username } = location.state?. username || 'Guest'{};` — **문법 오류**. `'Guest'{}`가 말이 안 되고, 구조분해(`{ username }`)와 기본값 처리가 뒤섞였다.
  - `margintop: "100px"` — React 인라인 스타일은 **camelCase**라 `marginTop`이어야 한다. 소문자면 그냥 무시된다.
- **해결**: 셋을 고치고, 선언만 해뒀던 `useNavigate`도 마저 연결했다.
  - `userLocation` → `useLocation`
  - `const username = location.state?.username || "Guest";` — 로그인에서 넘길 state의 username을 쓰되, 없으면 `"Guest"`.
  - `margintop` → `marginTop`
  - `const handleLogout = () => moveUrl("/")` — 로그아웃하면 로그인 화면(`/`)으로 돌아간다.
- **결과**: `/welcome`에 들어가면 `환영합니다, Guest님!`이 뜨고, 로그아웃을 누르면 `/`로 돌아온다. 라우터(`/`↔`/welcome`)가 실제로 도는 걸 확인했다. `?.`(옵셔널 체이닝)은 `location.state`가 없을 때 에러 없이 `undefined`를 주고, 그때 `|| "Guest"`가 기본값을 채운다 — 아직 로그인에서 username을 안 넘겨서 지금은 'Guest'로 뜨는 게 정상이다.
...
- 리액트 소개(11일차)→환경(12일차)→CSS·라우팅·통신 준비(오늘)로 프론트 한 바퀴를 돌았다. Welcome 오류를 잡아 `/`↔`/welcome` 이동까지 확인했으니, 다음은 로그인 성공 시 `/welcome`으로 username을 넘겨 이동시키는 배선과 백엔드 연동이다.
