...
> TL;DR: 25일차까지는 필요한 객체를 `new`로 직접 만들었다. 오늘은 그 자리가 사라진다. 객체를 만드는 일을 프레임워크에 넘기고, 코드는 "이게 필요하다"는 표시만 남긴다. 요청을 받는 자리와 데이터베이스에 붙는 자리에 같은 방식이 적용됐다.
...
- [예측] 컨트롤러에서 서비스를 거쳐 매퍼까지 한 줄기로 잇는 자리. 8절의 서비스 구현이 아직 `return null`이다.
- [예측] 리액트가 이 서버를 부르는 구간. 화면을 서버가 그리지 않는 3절의 오른쪽 칸이 그 자리다.

## 📌 다음에 해볼 것

- [ ] `@Autowired`·`@Resource`·`@Qualifier`·`@RequiredArgsConstructor`를 하나씩 바꿔 끼워 보고 어느 것이 무엇을 기준으로 고르는지 정리한다.
- [ ] `@Resource(name)`을 `"plain"`으로 바꿔 실행해 다른 구현이 불리는지 확인한다.
