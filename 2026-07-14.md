...
핵심은 **Java의 OS 독립성**이다. Java는 OS에는 독립적이고 JVM에는 의존적이다. 즉 Windows든 다른 OS든 상관없이, 그 위에 **JVM만 설치돼 있으면** 같은 Java 프로그램이 돌아간다. 강사가 판서한 그림이 이 구조를 보여준다.

![강사 판서 — OOP 4요소와 JDK/JVM/OS/하드웨어 계층](images/2026-07-14-01-jvm-layers.png)

그림을 아래에서 위로 읽으면 **하드웨어(h/w) → OS → JVM → lib → App** 순으로 쌓여 있다. 맨 아래 하드웨어와 OS는 컴퓨터마다 다르지만, 그 위에 JVM이 한 겹 깔리면서 위쪽의 App은 OS가 무엇인지 신경 쓰지 않아도 된다. 오른쪽에서 **JDK**가 이 묶음을 감싸고 있다. (JDK/JRE/JVM이 정확히 서로 어떻게 포함되는지는 헷갈려서 따로 확인했다 — 아래 트러블슈팅.)
...
- **문제**: 강의를 들으면서 나는 "JVM 안에 JRE가 있는 구조"라고 메모했는데, 판서 그림에서는 JDK가 전체를 감싸고 그 안에 JVM·lib·App이 있는 모양이라 방향이 헷갈렸다.
- **원인**: 세 개를 각각 따로 외우려다 보니 포함 방향이 뒤집혔다. 실제 관계는 내가 적은 것과 반대다.
- **해결**: 확인해 보니 포함 관계는 **JDK ⊃ JRE ⊃ JVM**이다.
  - **JVM**(Java Virtual Machine): `.class`를 실제로 실행하는 가상 머신. 가장 안쪽.
  - **JRE**(Java Runtime Environment): JVM + 실행에 필요한 표준 라이브러리(판서 그림의 `lib`). "실행"까지만 담당.
  - **JDK**(Java Development Kit): JRE + 개발 도구(`javac` 같은 컴파일러). "개발+실행"을 다 담당. 그래서 판서에서 JDK가 제일 바깥을 감싼다.
...
- 검증: 포함 관계는 판서 그림(JDK가 바깥, JVM이 안쪽)과 대조해서 내 메모("JVM 안에 JRE")가 반대였음을 확인. OS 독립성은 실습 실행 로그에 17버전 `java.exe`가 `.class`를 실행하는 명령으로 실제로 찍힌 걸로 맞춰봤다.
